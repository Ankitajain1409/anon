...
def dataloader(train, start_date, end_date,n):
    x_train =[]
    y_train = []
    n=n
    units_to_pad = n//2
    ...
    end_date= end_date
    train_df = train[["localminute","aggr","air1", 'refrigerator1', 'furnace1', 'clotheswasher1', 'dishwasher1']]
    train_df = train_df[(train_df["localminute"] > start_date) & (train_df["localminute"] < end_date)]
    x = train_df["aggr"].values*1000
    y_train = train_df[["air1", 'refrigerator1','furnace1','clotheswasher1','dishwasher1']].values*1000
