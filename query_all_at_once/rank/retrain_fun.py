...
def retrain(new_house, new_start,new_end, pool_houses, train, test, given_date, data_aggregated):
    ...
    for i in range(len(pool_houses)):
        ...
        for j in range(15):
            ...
            mc_test_sigma_list.append(test_sigma_mean)
        # test_mean = scaler_y.inverse_transform(y_hat[0])
        mc_test_sigma_list = np.array(mc_test_sigma_list)
        weights = np.array([0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0, 0.875, 0.75, 0.625, 0.5, 0.375, 0.25, 0.125])
        mc_test_sigma_weighted = mc_test_sigma_list * weights[:, np.newaxis]
        mc_test_sigma_weighted = mc_test_sigma_weighted.mean(axis = 0)
        mc_test_sigma_weighted_list.append( mc_test_sigma_weighted)
    print(mc_test_sigma_weighted_list)
    ranks_list = rankdata(mc_test_sigma_weighted_list, axis = 0)
    ranks_across_houses = np.array(ranks_list).sum(axis = 1)
    print(ranks_list)
    print(ranks_across_houses)
    max_house_id = np.argmax(ranks_across_houses)
    max_house = pool_houses[max_house_id]
    
    
    
    return max_house, max_house_id, rmse(y_test, test_mean), mae(y_test, test_mean), train, test, ranks_list, mc_test_sigma_weighted_list
